...
def load_kitti_calib(calib_txt_path: str):
    """
    calib.txt を読み取り、以下を返す:
      - P2_K, P3_K: 各カメラの内部行列(3x3)
      - P2, P3: 各カメラの [R|t] を 4x4 に埋めた行列
      - Tr: Velodyne -> Camera座標の 4x4 変換行列
    ファイル形式は data/kitti.py の KittiCalibHelper と同等を想定。
    """
    ...
    for ln in lines:
        ...
        if key in ['P2', 'P3']:
            ...
            T[0:3, 3] = np.array([tx, ty, tz], dtype=np.float32)
    ...
    assert P2 is not None and P3 is not None and Tr is not None, \
        "calib.txt must contain P2, P3, Tr"
    assert P2_K is not None and P3_K is not None, "Failed to parse K from P2/P3"
    return P2_K, P3_K, P2, P3, Tr


def load_frame_paths(root: str, seq: str, frame_idx: int, cam: str):
    """
    corri2p_data 配下の想定パスから、該当フレームのファイルパスを返す。
    cam: 'P2' or 'P3'
    """
    seq_dir = Path(root) / "sequences" / f"{int(seq):02d}"
    img_dir = seq_dir / f"img_{cam}"
    K_dir = seq_dir / f"K_{cam}"
    pc_dir = seq_dir / "pc_npy_with_normal"
...
# =========================
def main():
    parser = argparse.ArgumentParser(description="Visualize KITTI-like dataset (image + point cloud).")
    ...
    parser.add_argument("--cam", choices=["P2", "P3"], default="P2", help="Camera to use (P2 or P3)")
    ...
    # 1) ファイルパスを取得
    img_path, K_path, pc_path, calib_path = load_frame_paths(args.root, args.seq, args.frame, args.cam)
    ...
    # 4) calib 読み込み → Velodyne -> Camera の変換を組む
    P2_K, P3_K, P2, P3, Tr = load_kitti_calib(calib_path)
    if args.cam == "P2":
        T_cam_velo = (P2 @ Tr).astype(np.float32)  # 4x4
    else:
        T_cam_velo = (P3 @ Tr).astype(np.float32)
